...
export async function deletePersonaje(idPersonaje) {
    await client.connect()
    const personaje = await getPersonajeById(idPersonaje)
    const respuesta = await db.collection("personajes").updateOne(
        { _id: new ObjectId(idPersonaje) },
        {
            $set: { eliminado: true }
        }
    )
    if (personaje?.portada) await fs.unlink("uploads/" + personaje.portada)
    return respuesta
}
...
export async function editPersonajeById(personaje) {
    ...
    const personajeAnterior = await getPersonajeById(personaje._id)
    try {
        await client.connect()
        ...
        if (personaje?.portada) {
            await fs.unlink("uploads/" + personajeAnterior.portada)
        }
